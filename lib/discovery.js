...
Discovery.prototype.register = function (serviceOptions) {
    ...
    .error(function(error) {
        ...
        //todo: this is going to have to move from here to the service.on("added"... function below because
        //      at this point, sails hasn't lifted yet, so trying to make a request to the load balancer
        //      that is balancing requests for this service will fail because Sails isn't listening for any
        //      incoming requests yet.

        //if we are a service, check to see if we're behind a load balancer
        //if we are an adapter, then are not discoverable anyway, so no point in checking
        //if so, determine if we need to connect using the port or not
        if (process.env.LB_NAME && type == 'service') {
            //gonna use the url package to parse this string, so i'm appending 'http://' to make it look like a url
            var loadBalancer = url.parse('http://' + process.env.LB_NAME);
            var hostName = loadBalancer.hostname;
            var port = loadBalancer.port;
            if (port) {
                process.env.LB_NAME = hostName;
                process.env.LB_PORT = port;
            }
        }
        ...
        var advertisement = {
            name: serviceName,
            port: process.env.LB_PORT || sails.config.port
        };
