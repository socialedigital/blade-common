...
Discovery.prototype.initialize = function (sailsInstance) {
    ...
    portfinder.getPortAsync({port: sails.config.port})
    .then(function (port) {
        ...
        //check to see if we're behind a load balancer
        //if so, determine if we need to connect using the port or not

        //first try without the port - because that is most likely what will be in production
        if (process.env.LB_NAME) {
            var loadBalancerName = process.env.LB_NAME;
            Promise.join([
                request({
                    method : 'GET',
                    host   : process.env.LB_NAME,
                    service: serviceName,
                    path   : '/info'
                }),
                request({
            ...
            .then(function (noPortResult, withPortResult) {
                console.log(noPortResult.status);
                console.log(withPortResult.status);
            }).catch(function (error) {
                console.log(error);
...
Discovery.prototype.register = function (serviceOptions) {
    ...
    .error(function(error) {
        ...
        service.on("added", function (obj) {
            ...
            //only call /info for services and adapters - not for any other type
            return ((type == 'service' || type == 'adapter')
                ? request({
                    method : 'GET',
                    host   : info.host,
                    port   : info.port,
