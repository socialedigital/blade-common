...
function setUpMemoryLeakDetection(serviceInstance) {
    if (process.env.NODE_ENV != 'production') {         //don't watch for leaks in production
        sails.log.verbose('Watching for memory leaks...');
        var instance = serviceInstance;
        var memWatch = require('memwatch-next');
        var heapDump = require('heapdump');
        var heapDiff;
        memWatch.on('leak', function(info) {
            var name = serviceInstance.getName();
            ...
            var file = '/tmp/' + name + '-' + process.pid + '-' + Date.now() + '.heapsnapshot';
...
//check to see if we are being debugged and need to modify our environment
    if (debug) {
        if (sails.config.hasOwnProperty('debug')) {
            if (sails.config.debug.hasOwnProperty('startup')) {
                var path = require('path');
                var appDir = path.dirname(require.main.filename);
                sails.log.info(appDir);
                var environmentPath = path.resolve(appDir + '/config/' + sails.config.debug.startup)
                sails.log.info(environmentPath);
                sails.log.info('Adjusting environment variables according to ', environmentPath);
                try {
                    var debugStartup = require(environmentPath);
                    sails.config.redis.host = debugStartup.startupConfiguration.environment.REDIS_IP
                    sails.config.redis.port = debugStartup.startupConfiguration.environment.REDIS_PORT
                    process.env.NODE_ENV = debugStartup.startupConfiguration.environment.NODE_ENV
                    process.env.BLUEBIRD_DEBUG = debugStartup.startupConfiguration.environment.BLUEBIRD_DEBUG
                    process.env.DB_HOST = debugStartup.startupConfiguration.environment.DB_HOST
                    process.env.DB_PORT = debugStartup.startupConfiguration.environment.DB_PORT
                    process.env.DB_USER = debugStartup.startupConfiguration.environment.DB_USER
                    process.env.DB_PASSWORD = debugStartup.startupConfiguration.environment.DB_PASSWORD
                    process.env.DB_DATABASE = debugStartup.startupConfiguration.environment.DB_DATABASE
                }
                catch(error) {
                    sails.log.error("Couldn't load environment from " + environmentPath);
                }
            }
        }
    }
...
Discovery.prototype.register = function (serviceOptions) {
    ...
    .error(function(error) {
        ...
        service.on("added", function (obj) {
            ...
            //if the serice being added is this registered service, then don't do anything else
            if (name == serviceName) {
                return;
            }
            ...
            if (obj.advertisement.hasOwnProperty('debug')) {
                info.debug = obj.advertisement.debug;
            }
            ...
            .then(function(info) {
                ...
                if (addDiscoveryInfo(name, info)) {
                    sails.log.error(name + ' (' + info.host + ':' + info.port + ') is unavailable [' + error.message + ']');
                    wasAdded = true;
                }
                return wasAdded;
