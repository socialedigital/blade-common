...
function pickRandomService(serviceName) {
    ...
    var serviceList = _.reduce(nodes, function(list, service, hostName) {
...
var serviceCall = function(serviceName) {
    var self = this;
    var service = pickRandomService(serviceName);
    var serviceNotAvailable = Promise.reject('Service ' + serviceName + ' is not available.');
    return {
        get   : function (path) {
            if (service) {
                return request({
