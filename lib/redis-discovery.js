...
var Network = require('./discovery-network.js');
...
function Discover(sails, options, callback) {
    ...
    var settings = self.settings = {
        ...
        nodeTimeout    : options.nodeTimeout || 3000,
        masterTimeout  : options.masterTimeout || 4000,
    ...
    self.broadcast = new Network({sails: self.sails, key: settings.key});
     ...
     */
    self.evaluateHello = function (data, obj) {
...
Discover.prototype.join = function (channel, fn) {
    ...
    self.broadcast.on(channel, function (data, obj) {
        self.emit(channel, data, obj);
    });
